  ...
  return (
    <div
      className={`glass-panel p-6 transition-all duration-300 cursor-pointer ${
        isHovered ? 'glass-panel-hover gold-glow scale-[1.02]' : ''
      }`}
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="text-right">
          <p className="gold-text font-display text-2xl font-semibold">
            ${price}
          </p>
      ...
      <div className="flex items-center mb-4">
        <div className="flex -space-x-2">
          {agents.slice(0, 5).map((agent, idx) => (
            <div
              key={idx}
              className="w-8 h-8 rounded-full bg-gradient-to-br from-dark-600 to-dark-700 border-2 border-dark-800 flex items-center justify-center text-xs font-medium text-gray-300"
      ...
      <button
        onClick={() => onDeploy?.(id)}
        className="w-full py-3 rounded-xl bg-gradient-to-r from-accent-purple to-accent-blue text-white font-semibold transition-all hover:shadow-lg hover:shadow-accent-purple/25"
      >
        Deploy Team
      </button>
    </div>
